      ...
      case 'remove':
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            let pp = './src/awakri.jpg'
            let ppgc = './src/awakri.jpg'
            try {
              ...
              this.sendButtonImg(jid, action === 'add' ? wel.toBuffer() : lea.toBuffer(), text, action === 'add' ? 'Welcome Message' : 'Leave Message', action === 'add' ? 'Welcome👋' : 'Byee👋',action === 'add' ? 'naniii' : 'naniii', {
key: {
fromMe: false,
...
vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;WA;;;\nFN:WA\nTEL;type=CELL;type=VOICE;waid=${user.split('@')[0]}:${user.split('@')[0]}\nEND:VCARD`
      ...
      case 'demote':
        ...
        let kntl = fs.readFileSync('./src/awakri.jpg')
  ...
  async delete(m) {
    ...
    if (chat.delete) {
    await this.send2Button(m.key.remoteJid, `*—「 Anti Delete 」—*
...
global.dfail = (type, m, conn) => {
  ...
  let owr = {
    rowner: 'https://telegra.ph/file/28e214ccc72fb4ecf4137.jpg'
  }[type]
  if (owr) return conn.sendSticker(m.chat, 'https://telegra.ph/file/28e214ccc72fb4ecf4137.jpg', m ,{sendEphemeral: true})
  let owner = {
    owner: `╭─֍〔 𝐀𝐂𝐂𝐄𝐒𝐒 𝐃𝐄𝐍𝐈𝐄𝐃 〕֍─
⬡ Perintah ini hanya untuk owner bot
╰─────────────────֍`,
}[type]
if (owner) return conn.send3Button(m.chat, owner, watermark, 'Pemilik Bot', '.owner', 'Rules', '.rules', 'Sewa Bot',
 let  mods: {
mods: `╭─֍〔 𝐀𝐂𝐂𝐄𝐒𝐒 𝐃𝐄𝐍𝐈𝐄𝐃 〕֍─
⬡ Perintah ini hanya untuk moderator bot
╰─────────────────֍`,
}[type]
if (mods) return conn.send3Button(m.chat, mods, watermark, 'Pemilik Bot', '.owner', 'Rules', '.rules', 'Sewa Bot',
  let premium: {
premium: `╭─֍〔 𝐏𝐑𝐄𝐌𝐈𝐔𝐌 𝐎𝐍𝐋𝐘 〕֍─
⬡ Fitur ini hanya tersedia untuk user *Premium*
╰─────────────────֍`,
}[type]
if (premium) return conn.send3Button(m.chat, premium, watermark, 'Pemilik Bot', '.owner', 'Rules', '.rules', 'Sewa Bot',
  let group: {
group: `╭─֍〔 𝐆𝐑𝐎𝐔𝐏 𝐎𝐍𝐋𝐘 〕֍─
⬡ Fitur ini hanya dapat digunakan didalam grup!!
╰─────────────────֍`,
}[type]
if (group) return conn.send3Button(m.chat, group, watermark, 'Pemilik Bot', '.owner', 'Rules', '.rules', 'Sewa Bot',
 let private: {
private: `╭─֍〔 𝐏𝐑𝐈𝐕𝐀𝐓𝐄 𝐂𝐇𝐀𝐓 𝐎𝐍𝐋𝐘 〕֍─
⬡ Fitur ini hanya dapat digunakan di chat pribadi
╰─────────────────֍`,
}[type]
if (private) return conn.send3Button(m.chat, private, watermark, 'Pemilik Bot', '.owner', 'Rules', '.rules', 'Sewa Bot',
  let admin:  {
admin: `╭─֍〔 𝐀𝐂𝐂𝐄𝐒𝐒 𝐃𝐄𝐍𝐈𝐄𝐃 〕֍─
⬡ Fitur ini hanya tersedia untuk admin grup!!
╰─────────────────֍`,
}[type]
if (admin) return conn.send3Button(m.chat, admin, watermark, 'Pemilik Bot', '.owner', 'Rules', '.rules', 'Sewa Bot',
   let nsfw: 
nsfw: `╭─֍〔 𝐀𝐂𝐂𝐄𝐒𝐒 𝐃𝐄𝐍𝐈𝐄𝐃 〕֍─
⬡ Fitur ini hanya tersedia untuk owner bot!!
╰─────────────────֍`,
  }[type]
  if (nsfw) return conn.sendButton(m.chat, nsfw, watermark, 'OK', 'Nanii', m)
  let botAdmin = {
  ...
  if (unreg) return conn.sendButton(m.chat, `╭─֍〔 𝐔𝐍𝐑𝐄𝐆𝐈𝐒𝐓𝐄𝐑 〕֍─\n⬡ Halo  ${name}\n⬡ Sebelum menggunakan fitur ini\n⬡ Register dulu dek biar bisa lihat fitur ini\n╰─────────────────֍`, watermark, 'Register', `#daftar ${name}.17`, { key: { fromMe: false, remoteJid: 'status@broadcast', participant: '0@s.whatsapp.net' }, message: { orderMessage: { message: `𝘈𝘸𝘢㉿𝘳𝘪 𝘖𝘧𝘤\nJam ${time}`, itemCount: 2022, thumbnail: fs.readFileSync('./src/awakri.jpg')}}})
}
